...
bash: docker-init ## Connect to app container
	docker exec -it -w ${COMPOSE_WORKING_DIR} $(COMPOSE_PROJECT_NAME)_app /bin/bash -l

.PHONY: bash-root
bash-root: docker-init ## Connect to app container as root
	docker exec -it --user root -w ${COMPOSE_WORKING_DIR} $(COMPOSE_PROJECT_NAME)_app /bin/bash -l
